...
public class ProductoServiceImpl implements IProductoService {
    ...
    public List<Producto> getAllProductsByCategory(Long categoryId) {
        Categoria categoria = restTemplateConfig.restTemplate().getForObject("http://gateway-service:8080/categoria/" + categoryId, Categoria.class);
        if (categoria == null) {
    ...
    public Producto createProduct(Producto producto) {
        Categoria categoria = restTemplateConfig.restTemplate().getForObject("http://gateway-service:8080/categoria/" + producto.getCategoryId(), Categoria.class);
        if (categoria == null) {
            throw new RuntimeException("Categoria no encontrada");
        }
